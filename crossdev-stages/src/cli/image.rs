...
/// Reusable image build entry point so other CLI subcommands (e.g.
/// `update --apply`) can rebuild a board through the same pipeline as
/// `image build` without spawning a subprocess.
...
pub async fn run_build(
    ...
    let sb = ensure_crossdev(ws, sandbox, &board_cfg.arch, &board_cfg, mirror, None).await?;

    let tgt = match ws.resolve_target_for_arch(target, &board_cfg.arch) {
        Ok(td) => target::Target::open(td)?,
        Err(_) => {
            let name = target.unwrap_or(&board_cfg.arch).to_string();
